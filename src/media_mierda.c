...
avl_tree_node_t *nodo_verga = NULL;
...
static inline tipo_dato media_mierda_core(avl_tree_t *arbolin, int numerin,
	...
	bool ope_izq = falso;
	bool ope_par = falso;
	bool mover_izq = falso;
	bool mover_mierdia = falso;
	natural num_cacas = 0;
	...
	if (anadir) {
		...
		if (arbolin->nodos_realmente_en_arbol > 0
			...
			caca_log_debug("despues de borrar el nodo verga %u",
					nodo_verga?nodo_verga->llave:AVL_TREE_VALOR_INVALIDO);
			se_hizo_algo = verdadero;
			num_cacas--;
		}
	}
...
//	printList();
		...
		if (izo && head) {
			if (tam_caca == 1) {
				...
				mediano = actual->key;
				caca_log_debug("el medi ano %d (%u) el tam caca %u ", mediano,
						mitad_caca-1, tam_caca);
				if (!(tam_caca % 2)) {
					actual = head;
					conta_caca = 0;
					...
					caca_log_debug("el medi ano par  %d (%u)", mediano_par,
							mitad_caca-2);
